...
#     assert os.path.exists('env/log')
#     hdf5_files = []
#     for file in os.listdir('env/log'):
#         if file.endswith('.hdf5'):
#             hdf5_files.append(file)
    
#     for file in hdf5_files:
#         if env_name in file:
#             env_path = os.path.join('env/log', file)
#     print(f"Load {env_name} hdf5 file from {env_path}")
#     return env_path
...
def get_imitation_data(env_name):
    ...
    for file in hdf5_files:
        env_name = env_name.replace('Offline', 'Safety')
        if env_name in file:
            env_path = os.path.join('dataset', file)
    print(f"Load {env_name} hdf5 file from {env_path}")
    return env_path