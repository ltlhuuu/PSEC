...
class SIMFISOR(Agent):
    ...
    def update_actor(agent, batch: DatasetDict) -> Tuple[Agent, Dict[str, float]]:
        ...
        cost_exp_adv = jnp.exp((vc-qc) * 3.0)
        
        unsafe_weights = unsafe_condition * jnp.clip(cost_exp_adv, 1, 200) ## ignore vc >0, qc>vc
    ...
    def update(self, batch: DatasetDict):
        new_agent = self
        new_agent, actor_info = new_agent.update_actor(batch)
        new_agent, value_info = new_agent.update_vc(batch)
        new_agent, critic_info = new_agent.update_qc(batch)
