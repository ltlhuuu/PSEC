...
from jaxrl5.networks import MLP, Ensemble, StateActionValue, StateValue, DDPM, ddpm_sampler_eval, FourierFeatures, ddpm_sampler_eval_bc, cosine_beta_schedule, ddpm_sampler, MLPResNet, get_weight_decay_mask, vp_beta_schedule, lora_FourierFeatures, lora_MLP, LoRADense, LoRAResNet
from jaxrl5.networks.diffusion import dpm_solver_sampler_1st, vp_sde_schedule, ddpm_sampler_first
...
class FISOR(Agent):
    ...
    def create(
        ...
        eval1_maxq_weight: float = 0.07,
        eval1_minqc_weight: float = 0.07,
        eval2_maxq_weight: float = 0.07,
        eval2_minqc_weight: float = 0.07,
        eval3_maxq_weight: float = 0.07,
        eval3_minqc_weight: float = 0.07,
        eval4_maxq_weight: float = 0.07,
        eval4_minqc_weight: float = 0.07,
        eval5_maxq_weight: float = 0.07,
        eval5_minqc_weight: float = 0.07,
        eval6_maxq_weight: float = 0.07,
        eval6_minqc_weight: float = 0.07,
    ):

        rng = jax.random.PRNGKey(seed)
        rng, actor_key, lora_key, critic_key, value_key, safe_critic_key, safe_value_key = jax.random.split(rng, 7)
    ...
    def maxQ_update_actor(agent, batch: DatasetDict, train_lora: bool = True) -> Tuple[Agent, Dict[str, float]]:
        ...
        def actor_loss_fn(lora_model_params) -> Tuple[jnp.ndarray, Dict[str, float]]:
            ...
            actions, rng = ddpm_sampler_first(agent.q_lora_model.apply_fn,
                                        lora_model_params,
    ...
    def minQC_update_actor(agent, batch: DatasetDict, train_lora: bool = True) -> Tuple[Agent, Dict[str, float]]:
        ...
        def actor_loss_fn(lora_model_params) -> Tuple[jnp.ndarray, Dict[str, float]]:
            observations = batch['observations']
            actions, rng = ddpm_sampler_first(agent.qc_lora_model.apply_fn,
                                        lora_model_params,
                                        agent.score_model.apply_fn,
                                        agent.score_model.params,
                                        agent.T,
    ...
    # def maxQ_update_actor(self, batch: DatasetDict) -> Tuple[Agent, Dict[str, float]]:
    ...
    #     new_agent = self.replace(actor=actor, target_actor=target_actor, rng=rng)
    #     return new_agent, actor_info
    

    def eval_actions(self, observations: jnp.ndarray, train_lora: bool = False, eval_maxq_weight: float = 0.1, eval_minqc_weight: float = 0.1):
        rng = self.rng
        ...
        if self.sampling_method == 'ddpm':
            actions, rng, q1_eps_pred_lora_dis, q2_eps_pred_lora_dis, eps_pred_dis = ddpm_sampler_eval(self.q_lora_model.apply_fn,q_lora_params,
    ...
    def update_lora(self, batch: DatasetDict):
        ...
        new_agent, q_lora_actor_info = new_agent.maxQ_update_actor(batch, train_lora=True)

        new_agent, safe_critic_info = new_agent_bc.update_vc(batch)
        new_agent, safe_value_info = new_agent.update_qc(batch)
        new_agent, qc_lora_actor_info = new_agent.minQC_update_actor(batch, train_lora=True)


        # return new_agent, {**actor_info, **critic_info, **value_info, **safe_critic_info, **safe_value_info}
        return  new_agent_bc, new_agent, {**actor_info, **q_lora_actor_info, **qc_lora_actor_info, **critic_info, **value_info, **safe_critic_info, **safe_value_info} 
        # return  new_agent_bc, new_agent, {**actor_info, **lora_actor_info, **critic_info, **value_info} 
