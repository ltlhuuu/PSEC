...
from jaxrl5.networks import MLP, Ensemble, StateActionValue, StateValue, StateValue_Com,DDPM_Com, cosine_beta_schedule, ddpm_sampler, MLPResNet, get_weight_decay_mask, vp_beta_schedule
from jaxrl5.networks.net4lora_as import LoRA_FourierFeatures, LoRA_MLP, LoRAResNet
from jaxrl5.networks.diffusion import ddpm_sampler_eval_lora_one, ddpm_sampler_eval_alphas
...
class ComLoRALearner(Agent):
    score_model: TrainState
    target_score_model: TrainState
    critic: TrainState
    target_critic: TrainState
    value: TrainState
    composition: TrainState
    discount: float
    tau: float
    actor_tau: float
    ...
    def create(
        ...
        rng, actor_key, critic_key, value_key, composite_key = jax.random.split(rng, 5)
        actions = action_space.sample()
        observations = observation_space.sample()
        action_dim = action_space.shape[0]

        preprocess_time_cls = partial(LoRA_FourierFeatures,
                                      output_size=time_dim,
                                      learnable=True,
                                      pretrain_weights=pretrain_weights)

        cond_model_cls = partial(LoRA_MLP,
                                hidden_dims=(256, 256),
                                activations=mish,
        ...
        if actor_architecture == 'mlp':
            base_model_cls = partial(LoRA_MLP,
            ...
            actor_def = DDPM_Com(time_preprocess_cls=preprocess_time_cls,
                             cond_encoder_cls=cond_model_cls,
                             reverse_encoder_cls=base_model_cls)

        elif actor_architecture == 'ln_resnet':

            base_model_cls = partial(LoRAResNet,
                                     use_layer_norm=actor_layer_norm,
        ...
        composite_def = StateValue_Com(base_cls=critic_base_cls)
        composite_params = composite_def.init(composite_key, observations)["params"]
        composite_optimiser = optax.adam(learning_rate=value_lr)

        composition = TrainState.create(apply_fn=composite_def.apply,
                                  params=composite_params,
                                  tx=composite_optimiser)
    ...
    def update_actor_lora(agent, batch: DatasetDict) -> Tuple[Agent, Dict[str, float]]:
        rng = agent.rng
        key, rng = jax.random.split(rng, 2)
        time = jax.random.randint(key, (batch['actions'].shape[0], ), 0, agent.T)
        key, rng = jax.random.split(rng, 2)
    ...
    def update_actor_composition(agent, agent_lora, batch: DatasetDict) -> Tuple[Agent, Dict[str, float]]:
        rng = agent.rng
        key, rng = jax.random.split(rng, 2)
        time = jax.random.randint(key, (batch['actions'].shape[0], ), 0, agent.T)
        key, rng = jax.random.split(rng, 2)
        ...
        def alphas_loss_fn(composition_model_params) -> Tuple[jnp.ndarray, Dict[str, float]]:
            target_score_params = agent_lora.target_score_model.params
            alpha_as = agent.composition.apply_fn({'params': composition_model_params}, batch['observations'])
            eps_pred = agent.score_model.apply_fn({'params': target_score_params},
                                       batch['observations'],
                                       noisy_actions,
                                       time,
                                       alpha_as
            ...
            composition_loss = (((eps_pred - noise_sample) ** 2).sum(axis = -1) * weights).mean()

            return composition_loss, {'actor_loss': composition_loss, 'weights' : weights.mean(), 'alphas': alpha_as.mean(), 'alphas0': alpha_as[:,0][:,None].mean(), 'alphas1': alpha_as[:,1][:,None].mean()}

        grads, info = jax.grad(alphas_loss_fn, has_aux=True)(agent.composition.params)
        composition = agent.composition.apply_gradients(grads=grads)
        agent = agent.replace(composition=composition)
    ...
    def eval_actions_lora(self, observations: jnp.ndarray):
        rng = self.rng
        ...
        actions, rng = ddpm_sampler_eval_lora_one(
    ...
    def actor_update(self, batch: DatasetDict):
        new_agent = self
        new_agent, actor_info = new_agent.update_actor_weight(batch)
    ...
    def update_lora(self, batch: DatasetDict):
        ...
        new_agent, actor_info = new_agent.update_actor_lora(batch)
    ...
    def update_composition(self, agent_lora, batch: DatasetDict):
        ...
        new_agent, actor_info = new_agent.update_actor_composition(agent_lora, batch)
        new_agent, value_info = new_agent.update_v(batch)
        new_agent, critic_info = new_agent.update_q(batch)
