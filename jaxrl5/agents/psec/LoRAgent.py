...
from jaxrl5.networks import MLP, Ensemble, StateActionValue, StateValue, DDPM, DDPM_alpha, cosine_beta_schedule, ddpm_sampler, MLPResNet, get_weight_decay_mask, vp_beta_schedule
...
class LoRALearner(Agent):
    ...
    def update_actor_lora(agent, batch: DatasetDict) -> Tuple[Agent, Dict[str, float]]:
        ...
        if agent.actor_objective == "soft_adv":
            ...
            weights = jnp.where(adv >= (-0.01), agent.policy_temperature, 0)
    ...
    def update_actor_alphas(agent, agent_lora, batch: DatasetDict) -> Tuple[Agent, Dict[str, float]]:
        rng = agent.rng
        key, rng = jax.random.split(rng, 2)
        time = jax.random.randint(key, (batch['actions'].shape[0], ), 0, agent.T)
        key, rng = jax.random.split(rng, 2)
        ...
        def actor_loss_fn(value_params) -> Tuple[jnp.ndarray, Dict[str, float]]:
            target_lora_params = agent_lora.target_score_model.params
            # run_lora_params
            alpha_as = agent.value.apply_fn({"params": value_params}, batch["observations"])
            
            eps_pred = agent.score_model.apply_fn({'params': target_lora_params},
                                       batch['observations'],
                                       noisy_actions,
                                       time,
                                        alpha_as=alpha_as,
        ...
        grads, info = jax.grad(actor_loss_fn, has_aux=True)(agent.value.params)
        value = agent.value.apply_gradients(grads=grads)

        agent = agent.replace(value=value)

        # target_score_params = optax.incremental_update(
        #     value.params, agent.value.params, agent.actor_tau
        # )

        # target_score_model = agent.target_score_model.replace(params=target_score_params)

        new_agent = agent.replace(value=value, rng=rng)
    ...
    @jax.jit
    def update_alphas(self, agent_lora, batch: DatasetDict):
        new_agent = self

        new_agent, actor_info = new_agent.update_actor_alphas(agent_lora, batch)
        new_agent, value_info = new_agent.update_v(batch)
        new_agent, critic_info = new_agent.update_q(batch)
