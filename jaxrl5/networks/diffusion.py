...
#     pretrain_model_cls: Type[nn.Module]
#     lora_model_cls: Type[nn.Module]
#     @nn.compact
#     def __call__(self, *args, **kwargs) -> jnp.Any:
...
# @partial(jax.jit, static_argnames=('actor_apply_fn', 'act_dim', 'T', 'repeat_last_step', 'clip_sampler', 'training', 'train_lora'))
...
def ddpm_sampler_first(q1_actor_apply_fn, q1_actor_params, score_apply_fn, score_params, T, rng, act_dim, observations, alphas, alpha_hats, betas, sample_temperature, repeat_last_step, clip_sampler, rngs, training=False, weight=0.1):
    ...
    def fn(input_tuple, time):
        ...
        # eps_pred_lora_dis= jnp.linalg.norm(eps_pred_lora, axis=-1, keepdims=True)
        # # compute the eps_pred l2 distance
        # eps_pred_dis = jnp.linalg.norm(eps_pred, axis=-1, keepdims=True)
...
def ddpm_sampler_eval1(actor_apply_fn, actor_params, T, rng, act_dim, observations, alphas, alpha_hats, betas,
    ...
    return action_0, rng
# @partial(jax.jit, static_argnames=('actor_apply_fn', 'act_dim', 'T', 'repeat_last_step', 'clip_sampler', 'training'))
# def ddpm_sampler(actor_apply_fn, actor_params, T, rng, act_dim, observations, alphas, alpha_hats, betas,
#                  sample_temperature, repeat_last_step, clip_sampler, rngs, training=False, train_lora=False):
#     batch_size = observations.shape[0]
...
#         input_time = jnp.expand_dims(jnp.array([time]).repeat(current_x.shape[0]), axis=1)
#         eps_pred = actor_apply_fn({"params": actor_params}, observations, current_x, input_time,
#                                   rngs=rngs, training=training, train_lora=True)
...
#     return action_0, rng
# @partial(jax.jit, static_argnames=('actor_apply_fn', 'act_dim', 'T', 'repeat_last_step', 'clip_sampler', 'training'))
# def ddpm_sampler_eval(actor_apply_fn, actor_params, T, rng, act_dim, observations, alphas, alpha_hats, betas,
#                  sample_temperature, repeat_last_step, clip_sampler, training=False, train_lora=False):
#     batch_size = observations.shape[0]
...
@partial(jax.jit, static_argnames=('actor_apply_fn', 'act_dim', 'actor_apply_fn1', 'actor_apply_fn2', 'T', 'repeat_last_step', 'clip_sampler', 'training'))
def ddpm_sampler_eval_lora(actor_apply_fn, actor_params, actor_apply_fn1, actor_params1, actor_apply_fn2, actor_params2, T, rng, act_dim, observations, alphas, alpha_hats, betas,
                 sample_temperature, repeat_last_step, clip_sampler, training=False, train_lora=False):
    batch_size = observations.shape[0]
    ...
    def fn(input_tuple, time):
        ...
        # print the norm of the eps_pred
        print('eps_pred:',jnp.linalg.norm(eps_pred, axis=-1, keepdims=True).mean())

        energy_eps_pred = actor_apply_fn1({"params": actor_params1}, observations, current_x, input_time,
                                          training=training, train_lora=False)
        print('reward_eps:', jnp.linalg.norm(eps_pred, axis=-1, keepdims=True).mean())
        reward_eps_pred = actor_apply_fn2({"params": actor_params2}, observations, current_x, input_time, 
                                          training=training, train_lora=False)
        print('cost_eps:', jnp.linalg.norm(eps_pred, axis=-1, keepdims=True).mean())
        
        eps_pred = w0 * eps_pred + w1 * energy_eps_pred + w2 * reward_eps_pred
