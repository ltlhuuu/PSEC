...
@partial(jax.jit, static_argnames=('actor_apply_fn', 'act_dim', 'actor_apply_fn1', 'T', 'repeat_last_step', 'clip_sampler', 'training'))
def ddpm_sampler_ws(ws, actor_apply_fn, actor_params, 
                           actor_apply_fn1, actor_params1, 
    ...
    def fn(input_tuple, time):
        ...
        eps_pred = actor_apply_fn({"params": actor_params}, observations, current_x, input_time,
                                  training=training,)
        lora_eps_pred = actor_apply_fn1({"params": actor_params1}, observations, current_x, input_time,
                                          training=training,)
        # jax if else
        eps_pred = ws[:, 0][:, None] * eps_pred + ws[:, 1][:, None] * lora_eps_pred
...
@partial(jax.jit, static_argnames=('actor_apply_fn', 'act_dim', 'T', 'repeat_last_step', 'clip_sampler', 'training'))
def ddpm_sampler_eval_lora_one(actor_apply_fn, actor_params, T, rng, act_dim, observations, alphas, alpha_hats, betas,
                 sample_temperature, repeat_last_step, clip_sampler, training=False):
    batch_size = observations.shape[0]
