...
class LoRALayer(nn.Module):
    ...
    @nn.compact
    def __call__(self, x, alpha_as: None):
    ...
    def __call__(self, x: jnp.ndarray, alpha_as: jnp.ndarray = None, training: bool = False) -> jnp.ndarray:
        ...
        for i, size in enumerate(self.hidden_dims):
            ...
            x = LoRALayer(size, rank=self.rank, alpha_r=self.alpha_r,
                            ...
                            pretrain_bias=self.pretrain_weights['target_score_model']['params']['MLP_0'][f'Dense_{i}']['bias'])(x, alpha_as=alpha_as)
...
class LoRAResNet(nn.Module):
    ...
    def __call__(self, x: jnp.ndarray, alpha_as: jnp.ndarray = None, training: bool = False) -> jnp.ndarray:
        ...
        x = LoRALayer(self.hidden_dim, rank=self.rank, alpha_r=self.alpha_r,
                      ...
                      pretrain_bias=self.pretrain_weights['target_score_model']['params']['MLPResNet_0']['Dense_0']['bias'])(x, alpha_as=alpha_as)
        # add lora run
        for i in range(self.num_blocks):
            x = LoRAResnetBlock(self.hidden_dim, act=self.activations, use_layer_norm=self.use_layer_norm, dropout_rate=self.dropout_rate)(x, i, rank=self.rank, alpha_r=self.alpha_r, alpha_as=alpha_as, pretrain_weights=self.pretrain_weights, training=training)
        x = self.activations(x)
        # lora dense
        x = LoRALayer(self.out_dim, rank=self.rank, alpha_r=self.alpha_r,
                      ...
                      pretrain_bias=self.pretrain_weights['target_score_model']['params']['MLPResNet_0']['Dense_1']['bias'])(x, alpha_as=alpha_as)
...
class LoRAResnetBlock(nn.Module):
    ...
    @nn.compact
    def __call__(self, x, i,  rank, alpha_r,  pretrain_weights, alpha_as: jnp.ndarray = None, training: bool = False,):
        residual = x
        ...
        x = LoRALayer(self.features * 4, rank=rank, alpha_r=alpha_r,
                      ...
                      pretrain_bias=pretrain_weights['target_score_model']['params']['MLPResNet_0'][f'MLPResNetBlock_{i}']['Dense_0']['bias'])(x, alpha_as=alpha_as)
        ...
        x = LoRALayer(self.features, rank=rank, alpha_r=alpha_r,
                      ...
                      pretrain_bias=pretrain_weights['target_score_model']['params']['MLPResNet_0'][f'MLPResNetBlock_{i}']['Dense_1']['bias'])(x, alpha_as=alpha_as)
