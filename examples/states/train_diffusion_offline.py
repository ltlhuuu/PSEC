...
from jaxrl5.evaluation import evaluate, evaluate_bc, implicit_evaluate
...
def call_main(details):
    ...
    for i in tqdm(range(details['max_steps']), smoothing=0.1):
        sample = ds.sample_jax(details['batch_size'], keys=keys)
        # if i >= 2500001:
        #     agent, info = agent.update_maxQ(sample)
        # else:
        ...
        if i % details['log_interval'] == 0:
            ...
            # if i >= 2500001:
            #     _, val_info = agent.update_maxQ(sample)
            # else:
            #     _, val_info = agent.update(val_sample)
            if details['rl_config']['use_lora']:
                _, _, val_info = agent.update_lora(sample)
                _, _, val_info_bc = agent_bc.update_lora(sample)
            else:
                _, val_info = agent.update(sample)
            wandb.log({f"train/{k}": v for k, v in info.items()}, step=i)
            wandb.log({f"val/{k}": v for k, v in val_info.items()}, step=i)
            wandb.log({f"val_bc/{k}": v for k, v in val_info_bc.items()}, step=i)
        ...
        if i % details['eval_interval'] == 0 and i > 0:
            for inference_params in details['inference_variants']:
                ...
                eval_info = evaluate(
                    agent, env, details['eval_episodes'], save_video=details['save_video'], train_lora=True
                )
            ...
            for inference_params in details['inference_variants']:
                ...
                if 'binary' not in details['env_name']:
                    eval_info_bc["return"] = env.get_normalized_score(eval_info_bc["return"]) * 100.0
