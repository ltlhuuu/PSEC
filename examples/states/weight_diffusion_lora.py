...
def call_main(details):
    ...
    # if 'LoRASLearner' in details['rl_config']['model_cls']:
    else:
        env = gym.make(details['env_name'])
    ...
    if 'LoRA' in model_cls:
        details['pretrain_model'] = f"results/Pretrain/Pretrain_{details['env_name']}_ddpm_lora_/ddpm_lora_bc/model1000000.pickle"
        with open(details['pretrain_model'], 'rb') as file:
            config_dict['pretrain_weights'] = pickle.load(file)
            
    if model_cls == 'LoRASLearner':
        # TODO: optimize the location 
        ...
        # info1 = f"rank{details['rl_config']['rank']}-alpha{details['rl_config']['alpha_r']}-reward1-cost1"
        lora0_model = f"results/LoRA-{info1}/LoRA-{info1}_{details['env_name']}_ddpm_lora_/ddpm_lora/model50000.pickle"
        with open(lora0_model, 'rb') as lora_file:
             config_dict['lora0'] = pickle.load(lora_file)
             
        LoRA_pickle = f"model50000.pickle"
    ...
    if "BC" in model_cls:
        ...
        keys = None
    writer = SummaryWriter(f"log/PSEC/{details['timestamp']}_{details['env_name']}")
